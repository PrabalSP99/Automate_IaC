...
resource "local_file" "inventory" {
    content  = <<EOT
[aws_instance]
${aws_instance.my_instance[0].public_ip} ansible_user=ubuntu ansible_ssh_private_key_file={{secrets.AWS_SSH_KEY }} ansible_ssh_common_args='-o StrictHostKeyChecking=no'
EOT  
